...
- `src/services/blockers.js` implements: listBlockers, createBlocker, updateBlocker, deleteBlocker.
- All services perform basic error handling and support simple pagination (page/pageSize).

UI wiring:
- Existing pages call methods in `src/api/client.js`.
- That module now delegates to the Supabase services when `isSupabaseConfigured()` is true.
...
- reports: { id, content, status, week, blockers (json[] or text), created_at, updated_at, user_id? }
- blockers: { id, description, status, report_id, created_at, updated_at }
- For `blockers`, the `report_id` field is expected when associating with a report.
